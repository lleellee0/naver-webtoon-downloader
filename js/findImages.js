...
const findWebToonImages = (titleId, no, path, nid_aut, nid_ses, retryCount) => { // titleId is Webtoon's id, no is Sequence's id
  ...
  request({
    ...
    if (!error && response.statusCode == 200) {
      ...
      if(retryCount > 0)
        findWebToonImages(titleId, no, path, nid_aut, nid_ses, retryCount);
      console.log(error);
    }
  });
}
